  ...
  return (
    <div>
      ...
      <div className="lab-controls">
        ...
        <p>
          The diagonal step <M>{"(1,1)"}</M> visits {seen.size} of {L} points
          before repeating.{" "}
          {good
        ...
        <details open={twist}>
          ...
          <p>
            <Prose>
              {
                " In a direct product, the first coordinate always adds normally. In $D_{m} = C_{m} \\rtimes  C_{2}$, the second coordinate reverses that addition when it equals 1: "
              }
            </Prose>
          </p>
          ...
          <p>
            <Prose>
              {
                "Here $a,c$ are modulo $m$ and $b,d$ are modulo $2$. For $m>2$, $(0,1)(1,0)=(-1,1)$ whereas $(1,0)(0,1)=(1,1)$. Explore these products in the dihedral group machine."
              }
            </Prose>
          </p>
        </details>
...
export function PolynomialLab({ lesson }: { lesson: Lesson }) {
  ...
  const parse = (s: string) =>
    s.split(",").map((x) => (x.trim() ? Number(x.trim()) : NaN));
  let a = parse(raw),
    b = parse(rawB),
    valid =
      a.length <= 9 &&
  ...
  return (
    <div>
      <div className="lab-controls">
        <label>
          <Prose>{"Coefficients of $f$, constant term first"}</Prose>
        </label>
        <input
        ...
        />
        <label>
          <Prose>{"Coefficients of $g$, constant term first"}</Prose>
        </label>
        <input
        ...
        <Choice
          ...
          onChange={(v) => setP(+v)}
          options={[
            ["0", "Integer coefficients $\\mathbb Z$"],
            ["2", "Finite field $\\mathbb F_2$"],
            ["3", "Finite field $\\mathbb F_3$"],
            ["5", "Finite field $\\mathbb F_5$"],
            ["7", "Finite field $\\mathbb F_7$"],
            ["4", "Residue ring $\\mathbb Z/4\\mathbb Z$"],
          ]}
        />
        {!valid && (
          <p className="error-note">
            <Prose>
              {
                " Enter 1–9 integer coefficients, each between $-100$ and 100, separated by commas. "
              }
            </Prose>
          </p>
        )}
        {valid && <M block>{`f=${polyText(a)},\\quad g=${polyText(b)}`}</M>}
      </div>
      <div hidden={!valid}>
        <div
        ...
        <div className="lab-controls">
          ...
          {p ? (
            <p>
              <Prose>{" Roots of $f$ "}</Prose>
              <Prose>
                {p === 4 ? "modulo $4$" : `in $\\mathbb F_{${p}}$`}
              </Prose>{" "}
            ...
            <p>
              <Prose>{" The content of $f$ is "}</Prose>
              {content}.{" "}
              {content > 1
          ...
          >
            <summary>
              <Prose>{"Divide $f$ by $g$ / reduce modulo $g$"}</Prose>
            </summary>
            {b.every((x) => x === 0) ? (
              <p className="error-note">
              ...
              <p className="error-note">
                These coefficients exceed the exact integer range during
                division. Use smaller integer coefficients or a finite
                coefficient ring.
              </p>
            ) : invertible ? (
              <>
                ...
                <p>
                  ...
                  is <M>{polyText(remainder)}</M>
                  <Prose>
                    {
                      ". Each step cancels the current leading term. Thus $f$ has the same residue class as this remainder modulo $(g)$. "
                    }
                  </Prose>
                </p>
              ...
              <p className="error-note">
                <Prose>
                  {
                    "The divisor’s leading coefficient is not a unit in this ring. Division reduces the degree only when that coefficient is invertible. Over $\\mathbb Z$, choose a monic or negative-monic divisor."
                  }
                </Prose>
              </p>
            )}
          ...
          <details open={lesson.parameters?.mode === "eisenstein"}>
            ...
            <p>
              <Prose>
                {
                  " Eisenstein: let $f$ be a positive-degree integer polynomial. For a prime $p$ not dividing the leading coefficient, dividing all lower coefficients, and with $p^{2}$ not dividing the constant coefficient, $f$ is irreducible over $\\mathbb Q$. The polynomial $X^{4}+1$ fails this direct test, but $f(X+1)=X^{4}+4X^{3}+6X^{2}+4X+2$ passes for $p=2$. Translation is a ring automorphism, so irreducibility is preserved. "
                }
              </Prose>
            </p>
            {p === 0 && (
              <>
                ...
                <p>
                  Direct criterion:{" "}
                  {eisenstein(a) ? "passes" : "does not apply"}. After
                  translation:{" "}
                  <Prose>
                    {eisenstein(trim(shifted))
                      ? "passes; $f$ is irreducible over $\\mathbb Q$"
          ...
          <details>
            ...
            <p>
              <Prose>
                {
                  " Thus $X^{3}+1$ and $1-X$ represent the same element modulo $X^{2}+1$. Division by a monic polynomial works over any commutative unital coefficient ring. Over a field, divide by any nonzero polynomial. "
                }
              </Prose>
            </p>
          </details>
      ...
      {finite ? (
        <>
          ...
          <div className="lab-controls">
            ...
            <p>
              <Prose>
                {
                  " Addition is coefficient-wise modulo 2. Multiplication first expands and then replaces $\\alpha ^{2}$ by $\\alpha +1$. Every nonzero row permutes the three nonzero entries: every nonzero element is invertible. "
                }
              </Prose>
            </p>
            <p>
              <Prose>
                {
                  " There are four elements but the characteristic is 2. This field is not $\\mathbb Z /4\\mathbb Z$, where $2\\cdot 2=0$. "
                }
              </Prose>
            </p>
          </div>
        ...
        <>
          ...
          >
            ...
            <SvgMath x={290 + a * 17} y={195 - b * 17} fill="#69dbca">
              z
            </SvgMath>
          ...
          <div className="lab-controls">
            ...
            <p>
              <Prose>
                {
                  " Conjugation reflects across the real axis. The norm is squared distance from 0, and $N(zw)=N(z)N(w)$. The picture uses the Gaussian lattice $\\mathbb Z [i]$; quadratic integer lattices can be shifted, as the lesson explains. "
                }
              </Prose>
            </p>
            {d ? (
              <>
                ...
                <p>
                  ...
                  {rr * rr + ri * ri}
                  <Prose>{" < $N(w)=$"}</Prose>
                  {d}, so Euclidean division decreases the norm.
                </p>
              </>
            ) : (
              <p className="error-note">
